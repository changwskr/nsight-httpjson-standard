...
set "TASKS=:common-core:build :common-web:build :cc-service:bootWar"
...
set "OUT_FILE=!PROJECT_HOME!\!MODULE!\build\libs\cc.war"
echo.
...
echo   build.bat        Build cc-service cc.war
echo   build.bat clean  clean + build
echo   build.bat run    bootRun ^(port 8080^)
exit /b 0
