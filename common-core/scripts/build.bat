...
:gradle_ok
if not exist "!GRADLE!" (
    echo [common-core-build] gradle not found.
    exit /b 1
)
...
set "TASKS=:common-core:build"
if /i "%~1"=="clean" set "TASKS=clean !TASKS!"
if /i "%~1"=="run" set "TASKS=:!MODULE!:build"

echo [common-core-build] Stop Gradle daemons...
call gradle --stop >nul 2>&1

cd /d "!PROJECT_HOME!"
echo [common-core-build] !GRADLE! !TASKS!
call "!GRADLE!" !TASKS!
...
echo.
echo [common-core-build] Build output:
