...
:gradle_ok
if not exist "!GRADLE!" (
    echo [et-build] gradle not found.
    exit /b 1
)
...
set "TASKS=:common-core:build :common-web:build :common-etc:bootWar"
if /i "%~1"=="clean" set "TASKS=clean !TASKS!"
if /i "%~1"=="run" set "TASKS=:!MODULE!:bootWar"

echo [et-build] Stop Gradle daemons...
call gradle --stop >nul 2>&1

cd /d "!PROJECT_HOME!"
echo [et-build] !GRADLE! !TASKS!
call "!GRADLE!" !TASKS!
if errorlevel 1 exit /b %errorlevel%

if /i "%~1"=="run" exit /b 0

set "OUT_FILE=!PROJECT_HOME!\!MODULE!\build\libs\et.war"
echo.
echo [et-build] Build output:
if exist "!OUT_FILE!" (
    echo   [OK] et.war
    dir "!OUT_FILE!"
) else (
    echo   [MISSING] et.war - not found in !MODULE!\build\libs
    exit /b 1
)
exit /b 0
...
echo   build.bat        Build common-core/web + et.war
echo   build.bat clean  clean + build
echo   build.bat run    bootRun ^(port 8098^)
exit /b 0
