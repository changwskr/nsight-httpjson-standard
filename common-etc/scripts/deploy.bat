...
goto :webapps_ok
:webapps_missing
echo [et-deploy] webapps directory not found: !WEBAPPS!
exit /b 1
:webapps_ok
...
goto :gradle_found
:gradle_missing
echo [et-deploy] gradle not found.
exit /b 1
:gradle_found

cd /d "!PROJECT_HOME!"
echo [et-deploy] Building !SRC_WAR! ...
call "!GRADLE!" :common-core:build :common-web:build :common-etc:bootWar
if errorlevel 1 exit /b 1

set "SRC=!PROJECT_HOME!\!MODULE!\build\libs\!SRC_WAR!"
if not exist "!SRC!" (
    echo [et-deploy] WAR not found: !SRC!
    exit /b 1
)

if exist "!WEBAPPS!\!CTX!" rmdir /s /q "!WEBAPPS!\!CTX!" 2>nul

echo [et-deploy] Copying !DEST_WAR! to !WEBAPPS! ...
copy /Y "!SRC!" "!WEBAPPS!\!DEST_WAR!" >nul
echo   deployed !DEST_WAR!

echo.
echo [et-deploy] Verifying deployed WAR ...
if not exist "!WEBAPPS!\!DEST_WAR!" goto :verify_failed
echo   [OK] !DEST_WAR!
dir "!WEBAPPS!\!DEST_WAR!"
echo.
echo [et-deploy] Done. Tomcat running: /et redeploys automatically ^(~15s^).
exit /b 0

:verify_failed
echo [et-deploy] Verification failed: !DEST_WAR! not found in !WEBAPPS!
exit /b 1

:usage
echo Usage: deploy.bat
echo   Build et.war and deploy to Tomcat webapps ^(/et^)
echo.
echo Target webapps:
echo   !WEBAPPS!
