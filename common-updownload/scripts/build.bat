...
:gradle_ok
if not exist "!GRADLE!" (
    echo [ud-build] gradle not found.
    exit /b 1
)
...
set "TASKS=:common-core:build :common-web:build :common-updownload:bootWar"
if /i "%~1"=="clean" set "TASKS=clean !TASKS!"
if /i "%~1"=="run" set "TASKS=:!MODULE!:bootWar"

echo [ud-build] Stop Gradle daemons...
call gradle --stop >nul 2>&1

cd /d "!PROJECT_HOME!"
echo [ud-build] !GRADLE! !TASKS!
call "!GRADLE!" !TASKS!
if errorlevel 1 exit /b %errorlevel%

if /i "%~1"=="run" exit /b 0

set "OUT_FILE=!PROJECT_HOME!\!MODULE!\build\libs\ud.war"
echo.
...
if exist "!OUT_FILE!" (
    ...
    echo   [MISSING] ud.war - not found in !MODULE!\build\libs
    exit /b 1
)
exit /b 0
...
echo   build.bat        Build common-updownload ud.war
