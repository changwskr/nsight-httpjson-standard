...
:gradle_ok
if not exist "!GRADLE!" (
    echo [demo-ui-build] gradle not found.
    exit /b 1
)
...
set "TASKS=:demo-ui:bootJar"
if /i "%~1"=="clean" set "TASKS=clean !TASKS!"
if /i "%~1"=="run" set "TASKS=:!MODULE!:bootJar"

echo [demo-ui-build] Stop Gradle daemons...
call gradle --stop >nul 2>&1

cd /d "!PROJECT_HOME!"
echo [demo-ui-build] !GRADLE! !TASKS!
call "!GRADLE!" !TASKS!
if errorlevel 1 exit /b %errorlevel%

if /i "%~1"=="run" exit /b 0

set "OUT_FILE=!PROJECT_HOME!\!MODULE!\build\libs\demo-ui.jar"
echo.
...
if exist "!OUT_FILE!" (
    ...
    echo   [MISSING] demo-ui.jar - not found in !MODULE!\build\libs
    exit /b 1
)
exit /b 0
...
echo   build.bat        Build demo-ui demo-ui.jar
