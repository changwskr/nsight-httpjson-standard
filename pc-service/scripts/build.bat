...
:gradle_ok
if not exist "!GRADLE!" (
    echo [pc-build] gradle not found.
    exit /b 1
)
...
set "TASKS=:common-core:build :common-web:build :pc-service:bootWar"
if /i "%~1"=="clean" set "TASKS=clean !TASKS!"
if /i "%~1"=="run" set "TASKS=:!MODULE!:bootWar"

echo [pc-build] Stop Gradle daemons...
call gradle --stop >nul 2>&1

cd /d "!PROJECT_HOME!"
echo [pc-build] !GRADLE! !TASKS!
call "!GRADLE!" !TASKS!
if errorlevel 1 exit /b %errorlevel%

if /i "%~1"=="run" exit /b 0

set "OUT_FILE=!PROJECT_HOME!\!MODULE!\build\libs\pc.war"
echo.
echo [pc-build] Build output:
if exist "!OUT_FILE!" (
    echo   [OK] pc.war
    dir "!OUT_FILE!"
) else (
    echo   [MISSING] pc.war - not found in !MODULE!\build\libs
    exit /b 1
)
exit /b 0
...
echo   build.bat        Build pc-service pc.war
