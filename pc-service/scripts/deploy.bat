...
goto :webapps_ok
:webapps_missing
echo [pc-deploy] webapps directory not found: !WEBAPPS!
exit /b 1
:webapps_ok
...
goto :gradle_found
:gradle_missing
echo [pc-deploy] gradle not found.
exit /b 1
:gradle_found

cd /d "!PROJECT_HOME!"
echo [pc-deploy] Building !SRC_WAR! ...
call "!GRADLE!" :common-core:build :common-web:build :pc-service:bootWar
if errorlevel 1 exit /b 1

set "SRC=!PROJECT_HOME!\!MODULE!\build\libs\!SRC_WAR!"
if not exist "!SRC!" (
    echo [pc-deploy] WAR not found: !SRC!
    exit /b 1
)
...
:verify_failed
echo [pc-deploy] Verification failed: !DEST_WAR! not found in !WEBAPPS!
exit /b 1

:usage
echo Usage: deploy.bat
echo   Build pc.war and deploy to Tomcat webapps ^(/pc^)
echo.
echo Target webapps:
echo   !WEBAPPS!
